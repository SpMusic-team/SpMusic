...
你的职责是：[用 1-2 句话说明该 Agent 的长期稳定职责，不要写某个 Sprint 或版本的临时任务]。
...
- **当前阶段约束**：当前版本、当前 Sprint 或当前项目阶段的临时限制。
- **任务要求**：具体要交付的工作，应放入 `docs/sprint-plan.md` 或 `docs/tasks/*.md`，不要写进系统提示词的核心职责。

核心职责中不得写死以下内容，除非该 Agent 永久只负责该能力：

- 具体版本号，例如 `v0.1`。
- 具体 Sprint 任务，例如“替换模板页面”“展示假歌曲数据”。
- 临时实现策略，例如“只实现最小 command”。
- 当前阶段的禁止项，除非它属于长期职责边界。

如果需要保留阶段性限制，必须放入 `当前阶段约束` 章节。
...
在 `[当前阶段名称，例如 v0.1 项目地基阶段]`，你必须额外遵守：

1. [阶段性约束 1，例如本阶段只允许处理哪些范围]
2. [阶段性约束 2，例如本阶段明确不实现哪些能力]
3. [阶段性约束 3，例如本阶段需要优先验证哪些链路]

如果没有明确阶段性限制，可以保留本节并写明：当前暂无额外阶段约束，按已批准需求和任务执行。
...
| `docs/sprint-plan.md` | 当前 Sprint 目标、任务范围和执行计划 |
