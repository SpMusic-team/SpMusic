...
SP-006 将先接入 shadcn/ui 前端基底，SP-007 将把模板页面替换为 SpMusic 最小播放界面。为了避免界面实现过程中临时散落类型、假数据、文案、样式 token 和播放状态字符串，需要先建立一层很小的前端基础骨架，让后续界面实现只消费这些基础约束。
...
- 建立 UI-only 播放进度字段和演示频谱数据 fixture。
- 建立前端用户可见文案对象或简单模块。
...
- 不引入主题系统、主题导入或主题持久化。
- 不实现 Tauri command。
- 不实现真实音频播放、真实音频进度同步、真实频谱分析或 `HTMLAudioElement` 播放文件。
- 不新增依赖。
...
- 代码中存在最小 `PlayerState` 类型，包含当前歌曲标识、播放状态、歌曲列表、UI-only 播放进度和演示频谱数据。
- 代码中存在至少 5 条假歌曲 fixture。
- 代码中存在演示频谱 fixture 或等价前端演示数据。
- 用户可见文案集中在前端 `copy`、`texts`、`messages` 等对象或简单模块中。
- 播放状态类型只允许 `paused` / `playing`，不使用中文展示文案作为业务状态。
- 基础颜色、间距、圆角和状态色存在集中 CSS token 或等价集中定义。
- 未引入路由、全局状态库、i18n 框架、主题系统、Tauri command、真实音频播放、真实频谱分析或新依赖。
- `npm run lint` 通过。
- `npm run build` 通过。
